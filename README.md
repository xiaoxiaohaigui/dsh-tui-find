...
| 预览内 `n` / `N` | 跳到下一条 / 上一条命中（状态行显示「命中 i/total」） |
| `Alt+H` | 打开 / 关闭键位帮助面板 |
...
- **搜索**：大小写不敏感与高亮区间、CJK 子串、正则模式（逐命中区间、大小写敏感跟随、无效/过长/不安全模式拒绝、零宽样式安全）、`sinceMs` 时间窗（含边界）、工具摘要索引、repo/all 范围过滤（含子目录会话与容器边界）、结果幂等、多词 AND 查询（空白分词、引号短语、去重与 16 词上限、区间并集合并、正则整串不拆词、范围/时间窗组合）、拼音匹配（全拼/默认读音链/首字母、多音字双读音链、ü→v、表外字回退、高亮映射回汉字、开关关闭回归、正则隔离、大小写敏感语义、数据表完整性）。
...
- **场景接线（宿主真实渲染器）**：预览键位分层与输入吞掉、n/N 循环跳转与复制、PgDn 页长、窄头部单行截断、帮助面板开合。
