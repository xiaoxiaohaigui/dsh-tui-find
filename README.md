...
| `Alt+P` | classic 布局：打开全屏预览（锚定命中消息、命中词高亮）；阅读窗内 `↑↓` 按消息段跳转、`n`/`N` 命中跳转（循环回绕）、`PgUp`/`PgDn`/滚轮滚动 |
| `Alt+C` / `Alt+E` | 复制命中原文（预览内为光标处消息）/ 展开、收起当前会话全部命中 |
| `Alt+H` | 键位帮助面板 |
| `↵` / `Esc` | 恢复会话（二次确认）/ 清空查询、返回、退出场景 |

鼠标：左键选中、悬停高亮、滚轮按宿主同样的档距移动选择（一档 ≈ 3 行；在阅读窗内则按行滚动）；右键开上下文菜单（仅 0.10+ 宿主）：复制消息文本 / 复制会话日志路径 / 恢复该会话，预览内为复制光标处消息。

> 场景布局由 `layout` 配置选择：`split`（默认）在终端 ≥ 100 列时同屏显示"左侧列表 + 右侧会话内容阅读窗"（与 /resume 浏览器同风格）——选中项自动锚定右栏、右栏可独立滚动/复制/右键，`→` 聚焦右栏、`←` 回到列表（`Alt+P` 在 split 下不再使用；同一屏只有一处焦点——左栏持焦点时右栏不显示光标强调，聚焦右栏后左栏仍保留自己的高亮），窄终端自动回退单栏；`classic` 维持"单栏列表 + `Alt+P` 全屏预览"。两种形态下阅读窗都保证**命中词在视口内**：命中落在消息深处（视口装不下）时直接在命中行开窗，而不是停在看不到关键词的消息头。
...
- **拼音**（默认开，`pinyin` 可关）：纯字母词同时按拼音匹配汉字——全拼（`zhangsan` → 张三）、默认读音链（`zhongqing` → 重庆）、首字母（`zs` → 张三、`nrjb` → 南瑞继保，一个字母对一个字、可跨词连续）；全拼从音节起点匹配（不会把上一音节的尾字母接到下一字首字母），首字母则按你打字的样子连续拼接；多音字按全部读音参与、ü 按键盘习惯写作 v；内置 3500 常用字读音表，表外字按自身折叠；正则模式不扩展拼音。
...
测试覆盖（319 项）：帧链解析、扫描器（mtime 缓存复用、水位增量解码、世代命名枚举）、搜索（多词 AND / 正则 / 拼音含跨词首字母 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器（含命中落点开窗与输入即重定位）、键位帮助（按布局取词）、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发，含 split 分栏布局、←/→ 焦点移交、选择锚定、宽度回退与滚轮步幅）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
