...
- **扫描器**：双格式（zstd/明文）内容一致、mtime 缓存复用（二次扫描零解码）、同尺寸触碰零解码（边界验证）、水位增量解码（zstd/明文追加只解新增帧、撕裂尾帧补全不重不漏、缩水全量回退、水位日志 0600/0700 姿态与冷启动全量解码）、损坏样本容错、indexTools/indexThinking 开关、AbortSignal 中止。
