...
- **拼音搜索**（默认开启，`pinyin` 配置可关）：查询词为纯 ASCII 字母时，除原文子串外同时按拼音匹配汉字——全拼（`zhangsan` → 张三）、默认读音串联（`zhongqing` → 重庆、`changsha` → 长沙）与首字母（`zs` → 张三、`cq` → 重庆）；多音字按全部读音参与匹配（`chongqing` 与 `zhongqing` 都能命中重庆），ü 按键盘习惯写作 v（`lvse` 命中"绿色"）。内置《现代汉语常用字表》3500 字读音表（由 pinyin-pro 生成，`scripts/make-pinyin-data.mjs` 可复现），表外汉字按自身折叠、不影响其余匹配；命中高亮落回原汉字。"大小写敏感"只作用于英文原文，中文读音始终不区分大小写；正则模式不扩展拼音。
...
- **预览阅读器**：行布局与折行归属（含 CJK 宽字符）、光标行↔消息映射、命中跳转（前进/后退/越界）、滚动窗口跟随、命中区间折行映射（`wrapWidthRanges` 与 `wrapWidth` 等价性）。
- **键位帮助**：分区内容组装与窄列截断、Alt+H 开合接线。
- **场景接线（宿主真实渲染器）**：预览键位分层与输入吞掉、n/N 边界与复制、PgDn 页长、窄头部单行截断、帮助面板开合。
