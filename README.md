...
> 场景布局由 `layout` 配置选择：`split`（默认）在终端 ≥ 100 列时同屏显示"左侧列表 + 右侧会话内容阅读窗"（与 /resume 浏览器同风格）——选中项自动锚定右栏、右栏可独立滚动/复制/右键，`→` 聚焦右栏、`←` 回到列表（`Alt+P` 在 split 下不再使用），窄终端自动回退单栏；`classic` 维持"单栏列表 + `Alt+P` 全屏预览"。两种形态下阅读窗都保证**命中词在视口内**：命中落在消息深处（视口装不下）时直接在命中行开窗，而不是停在看不到关键词的消息头。
...
测试覆盖（317 项）：帧链解析、扫描器（mtime 缓存复用、水位增量解码、世代命名枚举）、搜索（多词 AND / 正则 / 拼音含跨词首字母 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器（含命中落点开窗与输入即重定位）、键位帮助（按布局取词）、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发，含 split 分栏布局、←/→ 焦点移交、选择锚定、宽度回退与滚轮步幅）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
