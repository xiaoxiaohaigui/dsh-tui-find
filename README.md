...
测试覆盖（298 项）：帧链解析、扫描器（mtime 缓存复用与水位增量解码）、搜索（多词 AND / 正则 / 拼音 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器、键位帮助、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发，含 split 分栏布局、焦点移交、选择锚定与宽度回退）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
