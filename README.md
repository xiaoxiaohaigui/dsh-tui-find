...
- **搜索**：大小写不敏感与高亮区间、CJK 子串、正则模式（逐命中区间、大小写敏感跟随、无效/过长/不安全模式拒绝、零宽样式安全）、`sinceMs` 时间窗（含边界）、工具摘要索引、repo/all 范围过滤（含子目录会话与容器边界）、结果幂等、多词 AND 查询（空白分词、引号短语、去重与 16 词上限、区间并集合并、正则整串不拆词、范围/时间窗组合）。
