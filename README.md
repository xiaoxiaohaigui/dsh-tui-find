...
| `↑↓` / `PgUp` `PgDn` | 条目间移动 / 翻页 |
| `Alt+P` | 阅读 / 预览：锚定命中消息、命中词高亮；阅读窗内 `↑↓` 按消息段跳转、`n`/`N` 命中跳转（循环回绕）、`PgUp`/`PgDn`/滚轮滚动；split 布局下在左右栏间移交焦点，classic 下打开全屏预览 |
| `Alt+C` / `Alt+E` | 复制命中原文（预览内为光标处消息）/ 展开、收起当前会话全部命中 |
| `Alt+H` | 键位帮助面板 |
| `↵` / `Esc` | 恢复会话（二次确认）/ 清空查询、返回、退出场景 |

鼠标：左键选中、悬停高亮、滚轮移动选择（预览内滚动阅读）；右键开上下文菜单（仅 0.10+ 宿主）：复制消息文本 / 复制会话日志路径 / 恢复该会话，预览内为复制光标处消息。

> 场景布局由 `layout` 配置选择：`split`（默认）在终端 ≥ 100 列时同屏显示"左侧列表 + 右侧会话内容阅读窗"（与 /resume 浏览器同风格）——选中项自动锚定右栏、右栏可独立滚动/复制/右键，`Alt+P` 在两栏间移交焦点，窄终端自动回退单栏；`classic` 维持"单栏列表 + `Alt+P` 全屏预览"。两种形态下阅读窗都保证**命中词在视口内**：命中落在消息深处（视口装不下）时直接在命中行开窗，而不是停在看不到关键词的消息头。
...
测试覆盖（315 项）：帧链解析、扫描器（mtime 缓存复用、水位增量解码、世代命名枚举）、搜索（多词 AND / 正则 / 拼音含跨词首字母 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器（含命中落点开窗与输入即重定位）、键位帮助、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发，含 split 分栏布局、焦点移交、选择锚定与宽度回退）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
