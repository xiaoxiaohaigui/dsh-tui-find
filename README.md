...
- **可读可复制**：只读预览锚定命中消息、命中词高亮，一键复制原文或日志路径。
...
| `↑↓` / `PgUp` `PgDn` | 条目间移动 / 翻页 |
| `Alt+P` | 只读预览：锚定命中消息、命中词高亮、显示日志路径；预览内 `↑↓` 按消息段跳转、`n`/`N` 命中跳转（循环回绕）、`PgUp`/`PgDn`/滚轮滚动 |
...
测试覆盖（283 项）：帧链解析、扫描器（mtime 缓存复用与水位增量解码）、搜索（多词 AND / 正则 / 拼音 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器、键位帮助、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
