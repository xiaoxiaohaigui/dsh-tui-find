...
> 输入 `/f` 时，建议列表中的 `/find` 行带随界面语言切换的中/英描述（宿主 `tuiCommandTrees` 命令树 provider，0.9.x 起即支持）。

> **后台预热索引**：启动约 10 秒后，插件在后台用与 `/find` 相同的扫描器预建一次索引，此后首次打开 `/find` 只需校验缓存、即刻出列表（`warmup` 配置可关）。0.10+ 宿主会在提示符上方显示一行"后台索引 n/m"进度，点按该行即可取消预热；0.9.x 宿主静默预热。索引只存内存、每次启动重建——预热并不减少总解码量，只是把不可避免的冷解码从"首次搜索时的前台停顿"挪到"启动后的空闲期"。
...
| 右键条目 | 打开上下文菜单（仅 0.10+ 宿主）：复制消息文本 / 复制会话日志路径 / 恢复该会话；预览窗内为复制光标处消息 |

> 鼠标支持依赖 dsh-TUI 的全屏鼠标跟踪。0.9.x 宿主运行时不派发右键事件，右键菜单仅在 0.10+ 宿主上注册（按注入 ui kit 的能力探测决定）；键盘快捷键始终是完整操作入口。
...
| 后台预热索引 | 开 / 关（默认开；关闭后首次 `/find` 需等待扫描） |
...
npm run verify:hosts  # 双宿主兼容矩阵：隔离副本换包，0.9.3 与 0.10.x 各跑一次 build + test
```
...
- **场景接线（宿主真实渲染器）**：预览键位分层与输入吞掉、n/N 循环跳转与复制、PgDn 页长、窄头部单行截断、帮助面板开合、结果流式渲染（扫描未完即渐进出条目）、扫描中空态（查询模式下显示读取提示而非误导性"无匹配"）、Alt+N 标题限定切换与恢复、右键菜单接线（右键开菜单并选中行、悬停移动高亮、键盘 ↑↓/Enter/Esc、backdrop 关闭不透传、条目点击激活、合并键块激活（↓↓+Enter 同块到达仍激活移动后的高亮）、0.9 代际门——指针用例经宿主 AlternateScreen 与 SGR 鼠标序列注入，右键派发用例仅在 0.10+ 宿主实跑，0.9 宿主标记跳过）。
- **宿主代际分派**：助手角色色键按注入 ui kit 的结构探测分派（0.9.x `claude` / 0.10+ `accent`，同色值的改名键），其余角色键代际稳定。
...
- **后台预热索引**：延迟启动（配置行到点复查、场景已开不启动、选项透传与场景 sweep 同构）、进度 store 的稳定快照语义（同值不重渲、落定归 idle）、场景打开让位（中止信号 + 尾随进度不复活）、点按取消、dispose 前后中止、失败降级单条告警；`tuiStatus` 结构软探测（无服务 / 0.9.x 形状 no-op、boot 拒绝重试落位、永久拒绝有界放弃且预热不受影响）、0.10 进度视图在真实宿主 kit 上的渲染（双语行、差量帧跟随、空闲零输出）。
