...
- **场景接线（宿主真实渲染器）**：预览键位分层与输入吞掉、n/N 循环跳转与复制、PgDn 页长、窄头部单行截断、帮助面板开合、结果流式渲染（扫描未完即渐进出条目）、扫描中空态（查询模式下显示读取提示而非误导性"无匹配"）、Alt+N 标题限定切换与恢复、右键菜单接线（右键开菜单并选中行、悬停移动高亮、键盘 ↑↓/Enter/Esc、backdrop 关闭不透传、条目点击激活、0.9 代际门——指针用例经宿主 AlternateScreen 与 SGR 鼠标序列注入，右键派发用例仅在 0.10+ 宿主实跑，0.9 宿主标记跳过）。
