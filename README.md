...
- **拼音**（默认开，`pinyin` 可关）：纯字母词同时按拼音匹配汉字——全拼（`zhangsan` → 张三）、默认读音链（`zhongqing` → 重庆）、首字母（`zs` → 张三）；多音字按全部读音参与、ü 按键盘习惯写作 v；内置 3500 常用字读音表，表外字按自身折叠；正则模式不扩展拼音。
...
测试覆盖（312 项）：帧链解析、扫描器（mtime 缓存复用、水位增量解码、世代命名枚举）、搜索（多词 AND / 正则 / 拼音 / 标题限定 / 时间窗 / 范围过滤）、预览阅读器（含命中落点开窗）、键位帮助、场景接线（真实宿主渲染器 + SGR 鼠标注入与右键派发，含 split 分栏布局、焦点移交、选择锚定与宽度回退）、宿主代际分派、事件清洗、显示宽度、准入与真实 fiber 挂载、启动竞态防护、后台预热索引与 `tuiStatus` 进度视图。
