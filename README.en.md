...
Test coverage (368 tests): frame chains, the scanner (mtime cache reuse, offset-watermark incremental decode, generation-named enumeration), search (multi-term AND / regex / pinyin with cross-word initials / title-only / time window / scope filtering), the fold build field-by-field against a frozen pre-refactor implementation, the fold's memory representation (what gets cached is a flat string, not a ConsString rope built one code point at a time — asserted as retained heap), the preview reader (hit-aware anchoring, re-landing as the query is typed, cursor-less line scrolling, window-relative n/N, the current-hit pick behind Alt+C, and a hint line that is fitted to the width and always one row), keyboard help (layout-aware vocabulary, the mouse fold badge), scene wiring (real host renderer with SGR mouse injection and right-click dispatch, including the split layout, the ←/→ focus handoff, selection anchoring, the width fallback, wheel step size, badge folding that never opens the row, and a fold leaving no hover tint on the next card), host-generation dispatch and the settings card (≤0.1.6 namespace registration, ≥0.1.7 taking the namespace from the Loader entry id with no registration, and card fields matching the live keys one-to-one), live config ref unwrapping (detected by the cosmokit protocol symbol, with the `{ get }` shape as a fallback), the resume confirm's merged key block (keys after Enter in the same chunk take the confirm branch), event sanitization, display width, admission and real-fiber mounting, boot-race hardening, and the background warm-up index (budgets, abort, the shape of the cache keys it fills, and the generated table's own lower bound) with its `tuiStatus` progress view.
